...
static int RecentArray_splice(Array *self, int index)
{
	if (index != -1)
	{
...
static void removeRecent(char *path)
{
	path += strlen(SDCARD_PATH); // makes paths platform agnostic
	int id = RecentArray_indexOf(favorites, path);
	if (id == -1)
	{ // add
		return;
	}
	...
	{ // remove
		RecentArray_splice(favorites, id);
	}
	saveRecents();
}
...
static Array *getRoot(void)
{
	...
	if (hasFavorites())
		Array_push(root, Entry_new(FAUX_FAVORITE_PATH, ENTRY_DIR));
...
#define VERSION_LINE_HEIGHT 24
				...
				GFX_blitButtonGroup((char *[]){"B", "ZURÜCK", NULL}, screen, 1);
			}
			else
			{
				// list
				if (total > 0)
				{
					...
					for (int i = top->start, j = 0; i < top->end; i++, j++)
					{
						...
						if (j == selected_row)
						{
							GFX_blitPill(ASSET_WHITE_PILL, screen, &(SDL_Rect){SCALE1(PADDING), SCALE1(PADDING + (j * PILL_SIZE)), max_width, SCALE1(PILL_SIZE)});
				...
				{
					GFX_blitMessage(font.large, "Ordner leer...", screen, NULL);
				}
				...
				{
					if (show_setting == 1)
						GFX_blitButtonGroup((char *[]){BRIGHTNESS_BUTTON_LABEL, "BRIGHTNESS", NULL}, screen, 0);
					else
						GFX_blitButtonGroup((char *[]){"MENU", "BRIGHTNESS", NULL}, screen, 0);
				}
				else if (can_resume)
				{
					GFX_blitButtonGroup((char *[]){"X", "WEITER", NULL}, screen, 0);
				}
				else
				{
					GFX_blitButtonGroup((char *[]){"POWER", "SLEEP", NULL}, screen, 0);
				...
				else
				{
					if (stack->count > 1)
					{
						GFX_blitButtonGroup((char *[]){"B", "ZURÜCK", "A", "ÖFFNEN", NULL}, screen, 1);
					}
					else
					{
						GFX_blitButtonGroup((char *[]){"A", "ÖFFNEN", NULL}, screen, 1);
					}
				}
			}
