...
SDL_Surface *GFX_init(int mode)
{
	...
	sinfo.enabled = 1;
	sinfo.disp_id = 2;
	if (ioctl(gfx.fd_fb, OWLFB_VSYNC_EVENT_EN, &sinfo) < 0)
...
void GFX_flip(SDL_Surface *screen)
{
	...
	if (gfx.vsync != VSYNC_OFF)
	{
		...
		if (gfx.vsync == VSYNC_STRICT || frame_start == 0 || SDL_GetTicks() - frame_start < FRAME_BUDGET)
		{ // only wait if we're under frame budget
			if (ioctl(gfx.fd_fb, OWLFB_WAITFORVSYNC, &_))
...
SDL_Surface *GFX_getBufferCopy(void)
...
{ // plat_sound_write / plat_sound_write_resample
	if (snd.frame_count == 0)
		return 0;
...
#define SLEEP_DELAY 30000
	if (now - cancel_start >= SLEEP_DELAY && POW_preventAutosleep())
		cancel_start = now;
...
void POW_powerOff(void)
{
	if (pow.can_poweroff)
	{
		char *msg = exists(AUTO_RESUME_PATH) ? "Spiel gespeichert,\nAusschalten..." : "Ausschalten...";
		GFX_clear(gfx.screen);
...
static void POW_waitForWake(void)
{
	...
	while (!wake)
	{
		...
		SDL_Delay(200);
		if (pow.can_poweroff && SDL_GetTicks() - sleep_ticks >= 120000)
		{ // increased to two minutes
			if (pow.is_charging)
				sleep_ticks += 60000; // check again in a minute
			else
				POW_powerOff(); // TODO: not working...
		}
	}
	return;
}
