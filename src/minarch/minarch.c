...
static struct Config
{
	...
	.frontend = (OptionList){
		...
		.options = (Option[]){
			[FE_OPT_SCALING] = {
				...
				.name = "Bildschirm-Skalierung",
				.desc = "Nativ verwendet eine ganzzahlige Skalierung.\nAspect verwendet das gemeldete Hauptseitenverhältnis.\nFullscreen erzeugt nicht-quadratische Pixel.",
				.default_value = 1,
				.value = 1,
				.count = 3,
			...
			[FE_OPT_SCANLINES] = {
				...
				.name = "Scanlinien/Gitter",
				.desc = "Simuliert Scanlinien (oder ein Pixelraster bei ungeraden Maßstäben).\nGilt nur für die native Skalierung.",
				.default_value = 0,
				.value = 0,
				.count = 2,
				.values = onoff_labels,
			...
			[FE_OPT_TEXT] = {
				...
				.name = "Text optimieren",
				.desc = "Bevorzugt eine konsistente Strichstärke bei der Hochskalierung von\nEinpixel-Linien unter Verwendung des Nearest Neighbour Scalers.\nErhöht die CPU-Last. Gilt nur für native Skalierung.",
				.default_value = 0,
				.value = 0,
				.count = 2,
				.values = onoff_labels,
			...
			[FE_OPT_TEARING] = {
				...
				.name = "Tearing verhindern",
				.desc = "Wartet auf vsync, bevor das nächste Bild gezeichnet wird. Lenient\nwartet nur, wenn innerhalb des Frame-Budgets. Strict wartet immer.",
			...
			[FE_OPT_OVERCLOCK] = {
				...
				.name = "CPU Modus",
				.desc = "CPU Über- oder untertakten zwischen maximaler Geschwindigkeit\noder maximaler Batterieleistung zu wechseln",
				.default_value = 1,
				.value = 1,
				.count = 3,
			...
			[FE_OPT_DEBUG] = {
				...
				.name = "Debug HUD",
				.desc = "Zeige FPS, CPU Last, Auflösung und Skalierungsinformationen.",
				.default_value = 0,
				.value = 0,
				.count = 2,
				.values = onoff_labels,
			...
			[FE_OPT_MAXFF] = {
				...
				.name = "Max Vorspulen Geschw.",
				.desc = "Vorspulen überschreitet nicht die gewählte Geschwindigkeit\n (kann aber je nach Spiel und Emulator geringer sein)",
				.default_value = 3, // 4x
				.value = 3,			// 4x
				.count = 8,
	...
	.shortcuts = (ButtonMapping[]){[SHORTCUT_SAVE_STATE] = {"Spielstand speichern", -1, BTN_ID_NONE, 0}, [SHORTCUT_LOAD_STATE] = {"Spielstand laden", -1, BTN_ID_NONE, 0}, [SHORTCUT_RESET_GAME] = {"Spiel neustarten", -1, BTN_ID_NONE, 0}, [SHORTCUT_CYCLE_SCALE] = {"Skalierung wechseln", -1, BTN_ID_NONE, 0}, [SHORTCUT_TOGGLE_SCANLINES] = {"Scanlinien umschalten", -1, BTN_ID_NONE, 0}, [SHORTCUT_TOGGLE_FF] = {"Vorspulen umschalten", -1, BTN_ID_NONE, 0}, [SHORTCUT_HOLD_FF] = {"Vorspulen", -1, BTN_ID_NONE, 0}, {NULL}},
};
...
static void setOverclock(int i)
{
	...
	{
	case 0:
		POW_setCPUSpeed(CPU_SPEED_POWERSAVE);
		break;
	case 1:
		POW_setCPUSpeed(CPU_SPEED_NORMAL);
		break;
	case 2:
		POW_setCPUSpeed(CPU_SPEED_PERFORMANCE);
		break;
	}
}
...
static void input_poll_callback(void)
{
	...
	for (int i = 0; config.controls[i].name; i++)
	{
		...
		if (PAD_isPressed(btn))
			buttons |= 1 << config.controls[i].retro;
	}
...
#define ACCEPTABLE_LOWER_BOUNDS 0.79 // but allow SotN's 512x240 scaled 1x2 to pass as core's 4/3 aspect ratio
	...
	if (use_nearest)
		if (show_scanlines)
			...
			renderer.scaler = optimize_text ? scaleNN_text : scaleNN;
	else
	{
		...
		if (show_scanlines)
		{
			...
			{
			case 6:
				renderer.scaler = scaleNN_scanline;
				break;
			case 5:
				renderer.scaler = scaleNN_scanline;
				break;
			case 4:
				renderer.scaler = scale4x_scanline;
				break;
			case 3:
				renderer.scaler = scale3x_grid;
				break;
			case 2:
				renderer.scaler = scale2x_scanline;
				break;
			default:
				renderer.scaler = scale1x_scanline;
				break;
			}
		}
		else
		{
			...
			{
			case 6:
				renderer.scaler = scale6x6_n16;
				break;
			case 5:
				renderer.scaler = scale5x5_n16;
				break;
			case 4:
				renderer.scaler = scale4x4_n16;
				break;
			case 3:
				renderer.scaler = scale3x3_n16;
				break;
			case 2:
				renderer.scaler = scale2x2_n16;
				break;
			default:
				renderer.scaler = scale1x1_n16;
				break;
...
static void selectScaler_AR(int width, int height, int pitch)
{
	...
	// else
	if (scale > 2)
		scale = 4; // TODO: pillar/letterboxing at 3x produces vertical banding (some kind of alignment issue?)
...
static void video_refresh_callback(const void *data, unsigned width, unsigned height, size_t pitch)
{
	...
	if (top_width)
		SDL_FillRect(screen, &(SDL_Rect){0, 0, top_width, DIGIT_HEIGHT}, RGB_BLACK);
	if (bottom_width)
		SDL_FillRect(screen, &(SDL_Rect){0, screen->h - DIGIT_HEIGHT, bottom_width, DIGIT_HEIGHT}, RGB_BLACK);
	...
	if (show_debug)
	{
		...
		if (fps_double)
			x = MSG_blitDouble(fps_double, x, y);
		...
		if (x > bottom_width)
			bottom_width = x; // keep the largest width because triple buffer
		...
		if (x > top_width)
			top_width = x; // keep the largest width because triple buffer
	}
...
static int OptionEmulator_optionDetail(MenuList *list, int i)
{
	...
	Option *option = OptionList_getOption(&config.core, item->key);
	if (option->full)
		return Menu_message(option->full, (char *[]){"B", "ZURÜCK", NULL});
	else
		return MENU_CALLBACK_NOP;
}
...
static MenuList OptionControls_menu = {
	.type = MENU_INPUT,
	.desc = "Drücke A zum zuweisen und X um die Zuweisung zu entfernen.",
...
static MenuList OptionShortcuts_menu = {
	.type = MENU_INPUT,
	.desc = "Drücke A zum zuweisen und X um die Zuweisung zu entfernen.\nUnterstützt einzelne Buttons und MENU+Button.",
...
static char *getSaveDesc(void)
{
	...
	{
	case CONFIG_NONE:
		return "Nutze Standards.";
		break;
	case CONFIG_CONSOLE:
		return "Nutze Konsolen-Standards.";
		break;
	case CONFIG_GAME:
		return "Nutze Spiel-Standards.";
		break;
	}
	return NULL;
}
...
static int OptionSaveChanges_onConfirm(MenuList *list, int i)
{
	...
	case 0:
	{
		...
		message = "Für die aktuelle\nKonsole gespeichert.";
		break;
	}
	case 1:
	{
		Config_write(CONFIG_WRITE_GAME);
		message = "Für das aktuelle\nSpiel gespeichert.";
		break;
	}
	default:
	{
		Config_restore();
		if (config.loaded)
			message = "Konsolen-Standards wiederhergestellt.";
		else
			message = "Standards wiederhergestellt.";
		break;
	}
	}
...
static MenuList options_menu = {
	...
	.items = (MenuItem[]){
		...
		{"Steuerung", .on_confirm = OptionControls_openMenu},
		{"Shortcuts", .on_confirm = OptionShortcuts_openMenu},
		{"Änderungen speichern", .on_confirm = OptionSaveChanges_openMenu},
		{NULL},
	}};
		...
		if (dirty)
		{
			...
			if (type == MENU_LIST)
			{
				...
				for (int i = start, j = 0; i < end; i++, j++)
				{
					...
					if (j == selected_row)
					{
						...
						GFX_blitPill(ASSET_BUTTON, screen, &(SDL_Rect){ox, oy + SCALE1(j * BUTTON_SIZE), w, SCALE1(BUTTON_SIZE)});
			...
			else if (type == MENU_FIXED)
			{
				...
				for (int i = start, j = 0; i < end; i++, j++)
				{
					...
					{
						// gray pill
						GFX_blitPill(ASSET_OPTION, screen, &(SDL_Rect){ox, oy + SCALE1(j * BUTTON_SIZE), mw, SCALE1(BUTTON_SIZE)});
					}
...
static void Menu_loop(void)
{
	...
	while (show_menu)
	{
		...
		if (dirty)
		{
			...
			{
				if (show_setting == 1)
					GFX_blitButtonGroup((char *[]){BRIGHTNESS_BUTTON_LABEL, "HELLIGKEIT", NULL}, screen, 0);
				else
					GFX_blitButtonGroup((char *[]){"MENU", "HELLIGKEIT", NULL}, screen, 0);
			}
			else
				GFX_blitButtonGroup((char *[]){"POWER", "SLEEP", NULL}, screen, 0);
			GFX_blitButtonGroup((char *[]){"B", "ZURÜCK", "A", "OKAY", NULL}, screen, 1);
			...
			for (int i = 0; i < MENU_ITEM_COUNT; i++)
			{
				...
				if (i == selected)
				{
					...
					{
						GFX_blitPill(ASSET_DARK_GRAY_PILL, screen, &(SDL_Rect){SCALE1(PADDING), SCALE1(oy + PADDING), screen->w - SCALE1(PADDING * 2), SCALE1(PILL_SIZE)});
					...
					// pill
					GFX_blitPill(ASSET_WHITE_PILL, screen, &(SDL_Rect){SCALE1(PADDING), SCALE1(oy + PADDING + (i * PILL_SIZE)), ow, SCALE1(PILL_SIZE)});
					text_color = COLOR_BLACK;
				}
				else
				{
					...
					SDL_BlitSurface(text, NULL, screen, &(SDL_Rect){SCALE1(2 + PADDING + BUTTON_PADDING), SCALE1(1 + PADDING + oy + (i * PILL_SIZE) + 4)});
					SDL_FreeSurface(text);
				}
...
#define WINDOW_RADIUS 4 // TODO: this logic belongs in blitRect?
				...
				else
				{
					...
					{ // has save but no preview
						GFX_blitMessage(font.large, "Keine Vorschau", screen, &preview_rect);
					}
					else
					{ // no save
						GFX_blitMessage(font.large, "Leer", screen, &preview_rect);
					}
				}
